...
import com.pinHouse.domain.diagnostic.rule.domain.entity.SupplyRentalCandidate;
...
public class BaseEligibilityRule implements Rule {
	...
	public RuleResult evaluate(EvaluationContext ctx) {
		...
		boolean ownsHouse = housingStatus.equals(
				com.pinHouse.domain.diagnostic.diagnosis.domain.entity.HousingOwnershipStatus.OWN_HOUSE);
		...
		boolean householdMemberOwnsHouse = housingStatus.equals(
				com.pinHouse.domain.diagnostic.diagnosis.domain.entity.HousingOwnershipStatus.HOUSEHOLD_MEMBER_OWNS_HOUSE);

		if (householdMemberOwnsHouse) {
			/// 무주택세대구성원이 필수인 경우 해당 후보 제거
			candidates.removeIf(c ->
				c.noticeType() == com.pinHouse.domain.housing.notice.domain.entity.NoticeType.NATIONAL_RENTAL ||
				c.noticeType() == com.pinHouse.domain.housing.notice.domain.entity.NoticeType.LONG_TERM_JEONSE ||
				c.noticeType() == com.pinHouse.domain.housing.notice.domain.entity.NoticeType.PUBLIC_RENTAL
			);
		}
